...
class DSFNet(CenterPoint):
    ...
    def __init__(self, 
                  ...
                  **kwargs):
        super(DSFNet, self).__init__(**kwargs)
        ...
        if frpn:
            self.frpn = builder.build_head(frpn)
            self.frpn_conv = frpn['frpn_conv']
            if self.frpn_conv:
                self.frpn_3x3conv = nn.Sequential(
                    nn.Conv2d(1, numC_Trans, kernel_size=3, padding=1),
                    nn.BatchNorm2d(80),
                    )
        ...
        # Seg Net
        self.seg_net = builder.build_head(seg_net) if seg_net else None
        ...
        batchnorm = nn.Batchnorm2d if self.forward_projection.nx[-1] == 1 else nn.BatchNorm3d
        ...
        self.history_keyframe_time_conv = nn.Sequential(
             conv(self.single_bev_num_channels + 1,
                     ...
                     padding=0,
                     stride=1),
             batchnorm(self.single_bev_num_channels),
        ...
        self.history_keyframe_cat_conv = nn.Sequential(
            conv(self.single_bev_num_channels * (self.history_cat_num + 1),
                    ...
                    padding=0,
                    stride=1),
            batchnorm(history_cat_conv_out_channels),
            nn.ReLU(inplace=True))
    ...
    def extract_img_bev_feat(self, img, img_metas, **kwargs):
        ...
        if self.with_specific_component('seg_net'):
            seg = self.seg_net(context)
            return_map['seg'] = seg
        ...
        if self.with_specific_component('frpn'): # not used in FB-OCC
            ...
            if self.frpn_conv:
                refine_feat = self.frpn_3x3conv(bev_mask_logit)
                refine_feat = refine_feat.view(B, self.numC_Trans, -1).permute(0, 2, 1)
                bev_mask = None
            else:
    ...
    def forward_train(self,
        ...
        if self.with_specific_component('seg_net'):
            losses_seg = self.seg_net.get_seg_loss(kwargs['gt_seg'], results['seg'])
