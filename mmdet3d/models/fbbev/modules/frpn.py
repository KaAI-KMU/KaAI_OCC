...
class FRPN(BaseModule):
    ...
    def __init__(
        ...
        self.dice_loss = DiceLoss(include_background=True, sigmoid=True, reduction='mean')
        self.ce_loss = torch.nn.BCEWithLogitsLoss(pos_weight=torch.tensor([2.13]))  # From lss
        self.mask_thre = mask_thre
        self.frpn_conv = frpn_conv

    def forward(self, input):
        if input.dim() == 5:
            bev_feat = input.max(dim=-1).values
        else: 
            bev_feat = input
        bev_mask = self.mask_net(bev_feat)            
