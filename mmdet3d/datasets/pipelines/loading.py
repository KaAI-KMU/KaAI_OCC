...
class LoadOccupancy(object):
    ...
    def __call__(self, results):
        ...
        results['foreground_occ'] = self.foreground_classes
        
        if self.fix_void:
            occupancy[occupancy<255] = occupancy[occupancy<255] + 1
            results['foreground_occ'] = [class_num+1 for class_num in results['foreground_occ']]
...
@PIPELINES.register_module()
class MakeBEVMask(object):
    """make GT BEVMask by pooling gt occupancy

    Args:
        results (dict): Result dict
    """
    def __init__(self, grid_config):
        self.bev_h = grid_config[0]
        self.bev_w = grid_config[1]
        
        
    def __call__(self, results):
        """Call functions make BEV_Mask for FRPN.

        Args:
            results (dict): Result dict.

        Returns:
            dict: The dict contains BEV_Mask.
        """
        h, w, z = results['gt_occupancy'].shape
        gt_bev_mask = torch.zeros_like(results['gt_occupancy'], dtype=torch.int)
        for fore_cls in results['foreground_occ']:
            gt_bev_mask[results['gt_occupancy'] == fore_cls] = 1
            
        gt_bev_mask = gt_bev_mask.to(dtype=torch.float32)
            
        gt_bev_mask, _ = torch.max(gt_bev_mask, dim=-1, keepdim=True)
        gt_bev_mask = gt_bev_mask.permute(2, 0, 1)
        
        assert h % self.bev_h == 0 and w % self.bev_w == 0
        kernel_h = h // self.bev_h
        kernel_w = w // self.bev_w
        gt_bev_mask = F.max_pool2d(gt_bev_mask.unsqueeze(0), kernel_size=(kernel_h, kernel_w))
        gt_bev_mask = gt_bev_mask.squeeze(0).squeeze(0)
...
class LoadSegmentationMap(object):
    ...
    def __init__(self, seg_path_key='samples_point_label', downsample=1, og_shape=(1600, 900), binary=True):
        self.seg_path_key = seg_path_key
        self.downsample = downsample
        self.segmap_shape = og_shape
        self.binary = binary # TODO: hardcoded to binary
    ...
    # def img_transform_core(self, img, resize_dims, crop, flip, rotate):
    #     img = self.to_pil(img)
    #     img = TF.resize(img, resize_dims, interpolation=Image.NEAREST) #BINARY
    #     img = TF.crop(img, crop[0], crop[1], crop[2] - crop[0], crop[3] - crop[1])
    #     if flip:
    #         img = TF.vflip(img)
    #     img = TF.rotate(img, rotate, interpolation=Image.NEAREST, center=None) #BINARY
    #     img = self.to_tensor(img).squeeze().T
        
    #     return img

    def img_transform_core(self, img, resize_dims, crop, flip, rotate):
        img = img.unsqueeze(0).unsqueeze(0)
        img = TF.resize(img, resize_dims, interpolation=TF.InterpolationMode.NEAREST)
        img = TF.crop(img, top=crop[0], left=crop[1], height=crop[2] - crop[0], width=crop[3] - crop[1])
        if flip:
            img = TF.vflip(img)
        img = TF.rotate(img, angle=rotate, interpolation=TF.InterpolationMode.NEAREST)
        img.clamp_(min=-1, max=1) # clamp 함수는 tensor의 값을 제한하는 데 사용
        img = img.squeeze(0).squeeze(0)

        return img
    

        # TODO code dongjak hwakin
    def points2segmap(self, points, height, width): # (4, N) -> (C, H, W) 
        height, width = height // self.downsample, width // self.downsample
        seg_map = -torch.ones((height, width), dtype=torch.float32)
        coor = torch.round(points[:, :2] / self.downsample)
        label = points[:, 2]
        label[label > -1] = 1
        label[label.to(torch.int32) == -1] = 0 
        kept1 = (coor[:, 0] >= 0) & (coor[:, 0] < width) & (
            coor[:, 1] >= 0) & (coor[:, 1] < height)
        coor, label = coor[kept1], label[kept1]

        coor = coor.to(torch.long)
        seg_map[coor[:, 1], coor[:, 0]] = label
        return seg_map

    def get_segmap(self, results):
        img_file_names = results['file_names']
        data = []
        post_rots, post_trans = results['img_inputs'][4], results['img_inputs'][5]
        for path, cam_name in zip(img_file_names, results['cam_names']):
            seg_file_path = path.replace('samples', self.seg_path_key).replace('jpg', 'npy')
            seg_map = np.load(seg_file_path)
            seg_map = torch.from_numpy(seg_map).T.to(torch.float32)
            points = torch.cat([seg_map[:, :2], seg_map[:, -1:]], 1)  # remove depth: x, y, label

            seg_img = self.points2segmap(points, self.segmap_shape[1], self.segmap_shape[0]).T # shape : 900, 1600
            _, resize_dims, crop, flip, rotate = results['img_augs'][cam_name]
            seg_img = self.img_transform_core(seg_img, resize_dims, crop, flip, rotate)
            data.append(seg_img.permute(1, 0))
        return torch.stack(data)
    
    def __call__(self, results):
        results['gt_seg'] = self.get_segmap(results)
        return results
