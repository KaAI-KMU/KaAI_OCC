...
BN = 'sync_bn' if sync_bn else 'BN3d'
        ...
        transformer=dict(
            ...
            encoder=dict(
                ...
                transformerlayers=dict(
                    ...
                    attn_cfgs=[
                        dict(
                            type='MultiScaleDeformableAttention_FRPN',
                            embed_dims=numC_Trans,
                            dropout=0.0,
                            num_levels=1),
                        dict(
...
train_pipeline = [
    ...
    dict(type='LoadOccupancy', ignore_nonvisible=True, fix_void=fix_void,
         occupancy_path=occupancy_path, foreground_classes=list(range(1, 1+len(class_names)))),
    dict(type='DefaultFormatBundle3D', class_names=class_names),
    dict(type='MakeBEVMask', grid_config=[bev_h_, bev_w_]),
    dict(
        type='Collect3D', keys=['img_inputs', 'gt_bboxes_3d', 'gt_labels_3d',  'gt_occupancy', 'gt_depth', 'gt_bev_mask', 'gt_seg'
                               ])
]
...
test_data_config = dict(
    ...
    ann_file=data_root + 'bevdetv2-nuscenes_infos_val1000.pkl')
